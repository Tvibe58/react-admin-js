 ...
 */
function request(url, params, method, options = { loading: true, mock: false, error: true, unAuth: false, token: 'token' }) {
  // let loadingInstance
  // 请求前loading
    ...
    instance({
      ...
      if (options.error) {
        if (error.response && error.response.status === 401) {
          message.error('请先登录', 5 * 1000);
        } else {
          message.error(error.message || '服务器错误，请稍后再试', 5 * 1000);
        }
      }
    }).finally(() => {
